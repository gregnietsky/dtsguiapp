...
<xsl:template name="smbconfigitem">
  <xsl:param name="smbitem"/>

  <xsl:choose>
    <xsl:when test="Config/Option[@option = $smbitem] != ''">
      <xsl:value-of select="concat('FileServer Config ',Config/Option[@option = $smbitem]/@option,' = ',Config/Option[@option = $smbitem],$nl)"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="concat('FileServer Config ',Config/Item[starts-with(.,$smbitem)],$nl)"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template match="FileServer">
  ...
  <xsl:call-template name="smbconfigitem">
    <xsl:with-param name="smbitem" select="'netbios name'"/>
  </xsl:call-template>
  <xsl:call-template name="smbconfigitem">
    <xsl:with-param name="smbitem" select="'server string'"/>
  </xsl:call-template>
  ...
  <xsl:call-template name="smbconfigitem">
    <xsl:with-param name="smbitem" select="'os level'"/>
  </xsl:call-template>
  <xsl:call-template name="smbconfigitem">
    <xsl:with-param name="smbitem" select="'preferred master'"/>
  </xsl:call-template>
  <xsl:call-template name="smbconfigitem">
    <xsl:with-param name="smbitem" select="'local master'"/>
  </xsl:call-template>
  <xsl:call-template name="smbconfigitem">
    <xsl:with-param name="smbitem" select="'domain master'"/>
  </xsl:call-template>
